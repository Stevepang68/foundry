...
impl<DB: Database> Inspector<DB> for CoverageCollector {
    fn initialize_interp(&mut self, interpreter: &mut Interpreter, _context: &mut DB) {
        get_or_insert_contract_hash(interpreter);
        self.insert_map(interpreter);
    }

    #[inline]
    fn step(&mut self, interpreter: &mut Interpreter, _context: &mut DB) {
...
impl CoverageCollector {
    ...
    fn insert_map(&mut self, interpreter: &mut Interpreter) {
        let Some(hash) = interpreter.bytecode.hash() else { eof_panic() };
...
#[cold]
#[inline(never)]
fn set_contract_hash(hash: &mut B256, bytecode: &revm::bytecode::Bytecode) {
