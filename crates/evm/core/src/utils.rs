...
>>>>>>> Stashed changes
    interpreter::{
        interpreter::EthInterpreter, CallInputs, CallScheme, CallValue, CreateInputs, Gas,
...
impl<CTX: ContextTr> PrecompileProvider<CTX> for MaybeOdysseyPrecompiles {
    ...
    fn warm_addresses(&self) -> Box<impl Iterator<Item = Address>> {
        let base = self.inner.warm_addresses() as Box<dyn Iterator<Item = Address>>;

        let iter = if self.odyssey {
            Box::new(base.chain(core::iter::once(*ODYSSEY_P256.address())))
        } else {
            base
        };
